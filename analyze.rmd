...
population_1990_2018$year <- row.names(population_1990_2018) %>% map(function(x) as.numeric(substring(x, 2)))
```
...
colnames(population_1990_2018)[colnames(population_1990_2018) == paste('V', as.character(ncol(population_1990_2018)))] <- '首都圏'

# Add a row for kanto
population_1990_2018[nrow(population_1990_2018) + 1,]<- colSums(population_1990_2018[kanto,])
colnames(population_1990_2018)[colnames(population_1990_2018) == as.character(ncol(population_1990_2018))] <- '関東'

# primacy
primacy <- t(population_1990_2018['首都圏',] / population_1990_2018['全県計',])
primacy <- cbind(primacy, t(population_1990_2018['東京都',] / population_1990_2018['全県計',]))
primacy <- cbind(primacy, t(population_1990_2018['関東',] / population_1990_2018['全県計',]))

primacy <- data.frame(primacy)
colnames(primacy) <- c('Shutoken Primacy', 'Tokyo Primacy', 'Kanto Primacy')

primacy$year <- row.names(primacy)
primacy$year <- 
library(ggplot2)
```
